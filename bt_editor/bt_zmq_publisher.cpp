...
std::atomic<bool> CustomPublisherZMQ::ref_count(false);

struct CustomPublisherZMQ::Pimpl
{
    Pimpl():
        context(1)
...
void setupTree(const Tree *tree);

using namespace BT;

CustomPublisherZMQ::CustomPublisherZMQ(
  ...
  , zmq_(new Pimpl())
{
    ...
    {
        throw LogicError("Only one instance of CustomPublisherZMQ shall be created");
    }
    ...
    thread_ = std::thread([this]() {
        while (active_server_)
        {
            ...
            {
                std::cout << "[CustomPublisherZMQ] just died. Exeption " << err.what() << std::endl;
...
void CustomPublisherZMQ::setupTree(const BT::Tree *tree){
    ...
    tree_ = tree;
    setupChangeLogger(tree_->rootNode());
    createStatusBuffer();
}
...
void CustomPublisherZMQ::createStatusBuffer()
{
...
void CustomPublisherZMQ::callback(Duration timestamp, const TreeNode& node, NodeStatus prev_status,
                            NodeStatus status)
{
